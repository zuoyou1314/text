...
@property (nonatomic, strong) NSArray *highlitedImages;
...
-(id)initWithPoint:(CGPoint)point titles:(NSArray *)titles imageNames:(NSArray *)images highlitedImages:(NSArray *)highlitedImages
{
    ...
    if (self) {
        ...
        self.highlitedImages = highlitedImages;
...
-(CGRect)getViewFrame
{
    ...
    if ([self.titleArray count] == [self.imageArray count]) {
        frame.size.width = 10 + 25 + 10 + frame.size.width + 40-19;
    }else{
        frame.size.width = 10 + frame.size.width + 40-19;
    }
...
//    cell.imageView.image = [UIImage imageNamed:[_highlitedImages objectAtIndex:indexPath.row]];
//    cell.textLabel.textColor = RGB(206, 206, 206);
